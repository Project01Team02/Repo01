...
function getMovieFromHistory(event) {
  ...
  var introBlock = document.getElementById("desc");
...
function getDetails(data) {
  ...
  for (var i = 0; i < ingredients.length; i++) {
    if (measurements[i] == null) {
      ...
      fullIngredients += `${ingredients[i].trim()} (${measurements[
        i
      ].trim()}), `;
    }
  }
...
function getMovieInfo(event) {
  ...
  fetch(`https://www.omdbapi.com/?apikey=333546da&t=${movieInfoPlus}`).then(
    function (response) {
      response.json().then(function (data) {
        console.log(data);
        destroyList();
